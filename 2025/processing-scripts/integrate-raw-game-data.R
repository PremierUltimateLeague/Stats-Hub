...
all.files <- list.files("2025/2025 Game Day Info/", pattern = ".csv",
...
read_stat <- function(path){
  ...
  match.week <- str_extract(path, "2025 Game Day Info/(.*?)_") |> str_remove("2025 Game Day Info/") |> str_remove("_") |> str_remove(coll("/"))
...
write_csv(possession.df, "2025/integ-data/Possessions.csv")
write_csv(player.stats.df, "2025/integ-data/Player-Stats.csv")
write_csv(defensive.blocks.df, "2025/integ-data/Defensive-Blocks.csv")
write_csv(points.df, "2025/integ-data/Points.csv")
write_csv(passes.df, "2025/integ-data/Passes.csv")
...
write_csv(game.coverage, "2025/stats/game-coverage.csv")
