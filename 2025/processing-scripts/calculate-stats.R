...
possession.df <- read_csv("2025/integ-data/Possessions.csv")

# this is per-game player statistics computed by Statto
player.stats.df <- read_csv("2025/integ-data/Player-Stats.csv") |>
  # convert meters to yards
  mutate(across(.cols = contains("(m)"), .fn = ~.x * 1.094)) |>
  rename_with(.fn = \(x)sub("[(]m[)]","(yd)",x))

defensive.blocks.df <- read_csv("2025/integ-data/Defensive-Blocks.csv")

points.df <- read_csv("2025/integ-data/Points.csv")

# this is 1-row per pass
##  use it to calculate the OVERALL pass and reception average distance
passes.df <- read_csv("2025/integ-data/Passes.csv") |>
  # convert meters to yards
  mutate(across(.cols = contains("(m)"), .fn = ~.x * 1.094)) |>
  rename_with(.fn = \(x)sub("[(]m[)]","(yd)",x))
...
################################################################################
# save stats
write_csv(team.stats.overall, "2025/stats/team-stats-overall.csv")
write_csv(team.stats.game, "2025/stats/team-stats-game.csv")
write_csv(player.stats.overall, "2025/stats/player-stats-overall.csv")
write_csv(player.stats.game, "2025/stats/player-stats-game.csv")
