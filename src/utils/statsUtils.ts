...
export function getLeadersWithRanks(
    ...
    for (let i = 0; i < sorted.length && result.length < n; i++) {
        const value = Number(sorted[i][stat]) || 0;
        if (i > 0 && value === (Number(sorted[i - 1][stat]) || 0)) {
    ...
    const lastValue = result.length > 0 ? Number(result[result.length - 1][stat]) : -1;
    for (let i = result.length; i < sorted.length; i++) {
        if ((Number(sorted[i][stat]) || 0) === lastValue) {
            result.push({ ...sorted[i], rank: result[result.length - 1].rank });
        } else {
            break;
        }
...
export function padLeadersToMatch(
    leadersByCategory: Record<string, StatRecord[]>,
    playersByCategory: Record<string, StatRecord[]>,
    categories: string[]
    ...
    const padded: Record<string, StatRecord[]> = {};
    ...
    for (const cat of categories) {
        ...
        while (result.length < maxCount && nextIndex < sorted.length) {
            const value = Number(sorted[nextIndex][cat]) || 0;
            const prevValue = Number(sorted[nextIndex - 1]?.[cat]) || 0;
            if (value === prevValue) {
 ...
 */
export function seasonHasStats(players: StatRecord[], statKeys: string[] = ['goals', 'assists', 'blocks']): boolean {
    return players.some(p =>
        statKeys.reduce((sum, key) => sum + (Number(p[key]) || 0), 0) > 0
    );
}
